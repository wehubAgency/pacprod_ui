...
const TargetTime = (props) => {
  ...
  const setActiveHourDay = (newWeek, selectDay, selectHour, active) => {
    return newWeek.map((day, index) => {
      if (index === selectDay) {
        return {
          ...day,
          isActive: !active
  ...
  const changeStateCell = throttle((selectDay, selectHour, selectCell) => {
    ...
    const newBufferChangeCell = [
      ...bufferChangeCell,
      { day: selectDay, hour: selectHour },
    ];
  ...
  const renderTime = (hours, selectDay, lastWeekElement = false) => {
